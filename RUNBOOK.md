...
- `codex_cli`：每个任务启动独立 `codex.cmd exec` 子进程，使用 `CODEX_HOME=profile_dir`、`--cd workspace_dir`、`--add-dir artifacts` 隔离并授权文件级交接；适合需要真实 Read/Write/Edit/Bash 行为的 niuma。Codex CLI 0.131.0 的自定义 provider 使用 Responses wire API；接 DeepSeek/普通中转站时请先启动 Moon Bridge 这类 `/v1/responses` 转发层，再把 worker 的 `base_url` 指向它。
  Windows 非交互执行写文件时，`workspace-write` 仍可能被 Codex exec policy 拦截；本地隔离工位建议使用 `--dangerously-bypass-approvals-and-sandbox`，并只把 `--cd` 指向该 niuma 的 workspace。
